...
- 代码中已包含详情页、扣费历史、通知 Worker、桌面 Widget、图标/壁纸选择、热力图和置顶能力。

### 2.2 已实现功能
- 首页卡片列表，默认按到期日升序。
- 订阅 CRUD：新增、查看详情、编辑、删除。
- 首页统计卡片：展示“本月订阅支出”、日均支出、已扣费/待扣费比例条；当前代码中总额暂为所有订阅金额直接求和，尚未按周期折算。
- 活跃度热力图：展示过去 12 周内创建/修改订阅的活跃情况。
- 状态筛选：全部、生效中、已失效；并展示即将续订/即将到期数量。
- 订阅卡片：Logo/预设图标、状态渐变边框、意向圆点、分类色点、金额、剩余天数、周期文案。
- 扣费确认弹窗：启动首页后扫描 7 天内到期的 ACTIVE 订阅，逐条确认续/不续/不确定。
- 扣费记录：确认操作会写入 `payment_history`，详情页展示历史记录。
- 详情页：顶部壁纸/状态色区域、信息摘要、展开编辑入口、历史扣费记录。
- 新增/编辑表单：名称、金额、周期、开始日期、到期日期、状态、意向、自动续费、图标、壁纸、备注。
- 到期日推算：默认按开始日期 + 周期天数计算，用户可手动覆盖。
- 图片能力：支持预设图标、自定义 Logo、自定义详情壁纸。
- 长按卡片操作：置顶/取消置顶、删除。
- 系统通知：每日 WorkManager 扫描明天到期的 ACTIVE 订阅并发通知。
...
- “本月订阅支出”的统计口径尚未完全产品化，需要补齐按周期折算规则。
...
|------|----------|--------|------|------------------|
| 生效中 | ACTIVE | `#2ECC71` 绿 | 正常使用中 | 新增默认值；用户也可手动选择 |
| 即将续订 | RENEWING | `#E67E22` 橙 | 用户确认会续订 | 扣费确认选择“是，续了”后写入，并推进到期日 |
| 即将到期 | EXPIRING | `#FF6B6B` 红 | 用户不续或即将失效 | 扣费确认选择“否，没续”后写入；也可手动选择 |
...
用户选择后的行为：
- **是，续了**：写入 `RENEWED` 扣费记录，状态改为 RENEWING，到期日加上当前周期天数。
...
### 6.1 视觉风格
- **设计语言**：现代极简卡片风格，轻量 Apple 感。
- **布局**：浅灰页面背景 + 白色卡片 + LazyColumn。
- **圆角**：卡片 20dp，Logo 12dp，Chip 30dp，按钮/内容块 12-16dp。
- **效果**：订阅卡片使用 1.5dp 薄渐变状态边框。
- **文字**：深色主文字 + 灰色辅助文字，金额突出但不过度装饰。
...
- `MainActivity` 处理 `subscriptiontracker://detail/{id}` 深链。
- `HomeViewModel` 聚合首页列表、统计、筛选、确认弹窗、热力图、删除和置顶状态。
...
| P2 | 数据导出 | CSV/PDF 导出 |
| P2 | 深色模式 | 跟随系统或手动切换 |
| P2 | 预设 Logo 优化 | 替换符号式图标为更统一的矢量/图片图标库 |
