...
8. **数据库迁移**：当前显式维护 `MIGRATION_2_3`，新增 `startDate` 与 `modifiedAt` 字段；未启用 destructive migration。
...
| RENEWING | 即将续订 | `#E67E22` | 用户在扣费确认中选择续订后写入，并把到期日推进一个周期 |
...
## 当前已实现功能
- 首页订阅列表，支持 4 种排序方式切换（按状态 / 按到期日 / 按加入时间 / 自定义）。
- 统计卡片：UI 文案为”本月订阅支出”；当前实现中总额为所有订阅 `amount` 直接求和，日均为总额除以订阅周期总天数，已扣费来自本月 `RENEWED` 记录，待扣费来自本月内到期的 ACTIVE 订阅。
- 过去 12 周订阅活跃度热力图，基于创建和修改时间生成。
- 状态筛选：全部、生效中、已失效；首页额外展示即将续订/即将到期数量。
- 订阅卡片：薄渐变状态边框、Logo/预设图标、意向圆点、分类色点、剩余天数、周期文案、金额。
- 新增/编辑表单：名称、金额、周期、开始日期、到期日期、状态、自动续费、意向、图标、壁纸、备注；日期字段支持手动输入和日历选择。
- 到期日自动推算：开始日期 + 周期天数；用户手动修改后标记为已覆盖。
- 预设 Logo 与自定义图片：预设包含音乐、视频、游戏、AI、云盘、工具、学习、健康、生活、其他；自定义图片复制到内部存储确保持久化。
- 详情页：壁纸区/状态色背景、摘要信息、展开编辑入口、历史扣费记录。
- 到期前 7 天扣费确认弹窗：逐条询问续/不续/不确定，并写入扣费记录。
- 过期自动流转：非自动续费 + 已过期 + ACTIVE 状态的订阅，进入首页时自动标记为 EXPIRING。
- 通知权限：Android 13+ 首次启动时自动弹出 `POST_NOTIFICATIONS` 权限请求。
- 长按卡片打开底部操作面板：置顶/取消置顶、删除。
- 删除确认弹窗；删除订阅时会清理关联的本地图片文件。
...
- **状态筛选叠加**: 状态筛选与排序独立工作，筛选后仍按所选排序方式排列。
